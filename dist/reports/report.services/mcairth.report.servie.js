...
let MCAIRTHReportService = class MCAIRTHReportService {
    ...
    findMCAIRTH(req) {
        return __awaiter(this, void 0, void 0, function* () {
            let result = [];
            try {
                const resultMCAIRTH = yield this.PatientformdetailsModel.aggregate([
                    {
                        $match: {
                            statusflag: "A",
                    ...
                    {
                        $addFields: {
                            MCTravelTHChkBoxOPD: { $arrayElemAt: [{
                                        $filter: {
                                            input: "$attributes",
                                            as: "vs",
                                            cond: { $eq: ["$$vs.attributename", "MCTravelTHChkBoxOPD"] }
                                        }
                                    }, -1]
                            }
                        }
                    ...
                    {
                        $addFields: {
                            MCTravelTHChkBoxIPD: { $arrayElemAt: [{
                                        $filter: {
                                            input: "$attributes",
                                            as: "vs",
                                            cond: { $eq: ["$$vs.attributename", "MCTravelTHChkBoxIPD"] }
                                        }
                                    }, -1]
                            }
                        }
                    ...
                    {
                        $addFields: {
                            MCTravelTHPatientAN: { $arrayElemAt: [{
                                        $filter: {
                                            input: "$attributes",
                                            as: "vs",
                                            cond: { $eq: ["$$vs.attributename", "MCTravelTHPatientAN"] }
                                        }
                                    }, -1]
                            }
                        }
                    ...
                    {
                        $addFields: {
                            MCTravelTHDateNowIPD: { $arrayElemAt: [{
                                        $filter: {
                                            input: "$attributes",
                                            as: "vs",
                                            cond: { $eq: ["$$vs.attributename", "MCTravelTHDateNowIPD"] }
                                        }
                                    }, -1]
                            }
                        }
                    ...
                    {
                        $addFields: {
                            MCTravelTHDateToIPD: { $arrayElemAt: [{
                                        $filter: {
                                            input: "$attributes",
                                            as: "vs",
                                            cond: { $eq: ["$$vs.attributename", "MCTravelTHDateToIPD"] }
                                        }
                                    }, -1]
                            }
                        }
                    ...
                    {
                        $addFields: {
                            MCTravelTHDateToCalendar: { $arrayElemAt: [{
                                        $filter: {
                                            input: "$attributes",
                                            as: "vs",
                                            cond: { $eq: ["$$vs.attributename", "MCTravelTHDateToCalendar"] }
                                        }
                                    }, -1]
                            }
                        }
                    ...
                    },
                    {
                        $group: {
                            ...
                            MCTravelTHChkBoxOPD: { "$push": "$MCTravelTHChkBoxOPD.textvalue" },
                            MCVisaTHDateOPD: { "$push": "$MCVisaTHDateOPD.textvalue" },
                            MCTravelTHChkBoxIPD: { "$push": "$MCTravelTHChkBoxIPD.textvalue" },
                            MCTravelTHPatientAN: { "$push": "$MCTravelTHPatientAN.textvalue" },
                            MCTravelTHDateNowIPD: { "$push": "$MCTravelTHDateNowIPD.textvalue" },
                            MCTravelTHDateToIPD: { "$push": "$MCTravelTHDateToIPD.textvalue" },
                            MCTravelTHDateToCalendar: { "$push": "$MCTravelTHDateToCalendar.textvalue" },
                    ...
                    },
                    {
                        $project: {
                            ...
                            MCTravelTHChkBoxOPD: { $arrayElemAt: ["$MCTravelTHChkBoxOPD", -1] },
                            MCVisaTHDateOPD: { $arrayElemAt: ["$MCVisaTHDateOPD", -1] },
                            MCTravelTHChkBoxIPD: { $arrayElemAt: ["$MCTravelTHChkBoxIPD", -1] },
                            MCTravelTHPatientAN: { $arrayElemAt: ["$MCTravelTHPatientAN", -1] },
                            MCTravelTHDateNowIPD: { $arrayElemAt: ["$MCTravelTHDateNowIPD", -1] },
                            MCTravelTHDateToIPD: { $arrayElemAt: ["$MCTravelTHDateToIPD", -1] },
                            MCTravelTHDateToCalendar: { $arrayElemAt: ["$MCTravelTHDateToCalendar", -1] },
                ...
                result = resultMCAIRTH;
            }
            catch (error) {
                this.logger.error('findMCAIRTH error:', error);
            }
            return result;
        });
    }
};
MCAIRTHReportService = __decorate([
    common_1.Injectable(),
    __param(1, mongoose_2.InjectModel('patientformdetails')),
    __metadata("design:paramtypes", [logger_1.ConsoleLogger,
        mongoose_1.Model])
], MCAIRTHReportService);
exports.MCAIRTHReportService = MCAIRTHReportService;
//# sourceMappingURL=mcairth.report.servie.js.map