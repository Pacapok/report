...
@Injectable()
export class STATCODERICD9TOPRECORDOPDReportService implements STATCODERICD9TOPRECORDOPDReportService {
    constructor(
        private logger: ConsoleLogger,
        @InjectModel('patientprocedures') private readonly PatientproceduresModel: Model<IPatientprocedures>,        
    ) { }

    async findSTATCODERICD9TOPRECORDOPD(req: STATCODERICD9Req): Promise<any> {
        let result = [];
        try {
            const resultfindSTATCODERICD9TOPRECORDOPD = await this.PatientproceduresModel.aggregate([
                {
                    $match: {
                        orguid: 
            ...
            result = resultfindSTATCODERICD9TOPRECORDOPD
        } catch (error) {
            this.logger.error('findSTATCODERICD9TOPRECORDOPD error:', error);
        }
        return result;
    }
}
