...
@Injectable()
export class STATCODERICD10OPDReportService implements STATCODERICD10OPDReportService {
    constructor(
        private logger: ConsoleLogger,
        @InjectModel('diagnoses') private readonly DiagnosesModel: Model<IDiagnoses>,        
    ) { }

    async findSTATCODERICD10OPD(req: STATCODERICD10Req): Promise<any> {
        let result = [];
        try {
            const resultfindSTATCODERICD10OPD = await this.DiagnosesModel.aggregate([
                {
                    $match: {
                        orguid: 
            ...
            result = resultfindSTATCODERICD10OPD
        } catch (error) {
            this.logger.error('findSTATCODERICD10OPD error:', error);
        }
        return result;
    }
}
