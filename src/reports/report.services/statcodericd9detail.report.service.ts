...
@Injectable()
export class STATCODERICD9DETAILReportService implements STATCODERICD9DETAILReportService {
    constructor(
        private logger: ConsoleLogger,
        @InjectModel('patientprocedures') private readonly PatientproceduresModel: Model<IPatientprocedures>,   
    ) { }

    async findSTATCODERICD9DETAIL(req: STATCODERICD9Req): Promise<any> {
        let result = [];
        try {
            const resultfindSTATCODERICD9DETAIL = await this.PatientproceduresModel.aggregate([
                {
                    $match: {
                        orguid:
                ...
                },
                {
                    $project: {
                        ...
                        VISITDATE: 
                        //"$patientvisits.startdate",
                            { $dateToString: { format: "%Y-%m-%d %H:%M", date: "$patientvisits.startdate", timezone: "+07:00", onNull: "-" } },
                        ...
                        CAREPROVIDERS: "$careproviders.name",
                ...
                },
                {
                    $project: {
                        ...
                        CAREPROVIDERS: "$_id.CAREPROVIDERS",
            ...
            result = resultfindSTATCODERICD9DETAIL
        } catch (error) {
            this.logger.error('findSTATCODERICD9DETAIL error:', error);
        }
        return result;
    }
}
