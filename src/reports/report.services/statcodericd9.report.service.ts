...
@Injectable()
export class STATCODERICD9ReportService implements STATCODERICD9ReportService {
    constructor(
        private logger: ConsoleLogger,
        @InjectModel('patientprocedures') private readonly PatientproceduresModel: Model<IPatientprocedures>,        
    ) { }

    async findSTATCODERICD9(req: STATCODERICD9Req): Promise<any> {
        let result = [];
        try {
            const resultfindSTATCODERICD9 = await this.PatientproceduresModel.aggregate([
                {
                    $match: {
                        orguid: 
            ...
            result = resultfindSTATCODERICD9
        } catch (error) {
            this.logger.error('findSTATCODERICD9 error:', error);
        }
        return result;
    }
}
