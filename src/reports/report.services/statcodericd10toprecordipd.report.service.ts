...
@Injectable()
export class STATCODERICD10TOPRECORDIPDReportService implements STATCODERICD10TOPRECORDIPDReportService {
    constructor(
        private logger: ConsoleLogger,
        @InjectModel('diagnoses') private readonly DiagnosesModel: Model<IDiagnoses>,        
    ) { }

    async findSTATCODERICD10TOPRECORDIPD(req: STATCODERICD10Req): Promise<any> {
        let result = [];
        try {
            const resultfindSTATCODERICD10TOPRECORDIPD = await this.DiagnosesModel.aggregate([
                {
                    $match: {
                        orguid: 
            ...
            result = resultfindSTATCODERICD10TOPRECORDIPD
        } catch (error) {
            this.logger.error('findSTATCODERICD10TOPRECORDIPD error:', error);
        }
        return result;
    }
}
