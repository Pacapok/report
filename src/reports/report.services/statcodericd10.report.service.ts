...
@Injectable()
export class STATCODERICD10ReportService implements STATCODERICD10ReportService {
    constructor(
        private logger: ConsoleLogger,
        @InjectModel('diagnoses') private readonly DiagnosesModel: Model<IDiagnoses>,        
    ) { }

    async findSTATCODERICD10(req: STATCODERICD10Req): Promise<any> {
        let result = [];
        try {
            const resultfindSTATCODERICD10 = await this.DiagnosesModel.aggregate([
                {
                    $match: {
                        orguid: 
            ...
            result = resultfindSTATCODERICD10
        } catch (error) {
            this.logger.error('findSTATCODERICD10 error:', error);
        }
        return result;
    }
}
