...
@Injectable()
export class REFERTHReportService implements REFERTHReportService {
    constructor(
...
//Referal Report Thai
    async findREFERTH(req: REFERReq): Promise<any> {
    let result = [];
    try {
        const resultfindREFERTH = await this.PatientformdetailsModel.aggregate([
            {
                $match:{
                    statusflag : "A",
