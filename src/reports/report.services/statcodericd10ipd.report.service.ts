...
@Injectable()
export class STATCODERICD10IPDReportService implements STATCODERICD10IPDReportService {
    constructor(
        private logger: ConsoleLogger,
        @InjectModel('diagnoses') private readonly DiagnosesModel: Model<IDiagnoses>,        
    ) { }

    async findSTATCODERICD10IPD(req: STATCODERICD10Req): Promise<any> {
        let result = [];
        try {
            const resultfindSTATCODERICD10IPD = await this.DiagnosesModel.aggregate([
                {
                    $match: {
                        orguid: 
                ...
                {
                    $sort: { "_id.ICD10DEPT": 1, "records": -1,"TOTALBILLS1":-1 }
                }
        ]).exec()
            result = resultfindSTATCODERICD10IPD
        } catch (error) {
            this.logger.error('findSTATCODERICD10IPD error:', error);
        }
        return result;
    }
}
