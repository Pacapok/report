...
@Injectable()
export class STATCODERICD9TOPRECORDIPDReportService implements STATCODERICD9TOPRECORDIPDReportService {
    constructor(
        private logger: ConsoleLogger,
        @InjectModel('patientprocedures') private readonly PatientproceduresModel: Model<IPatientprocedures>,        
    ) { }

    async findSTATCODERICD9TOPRECORDIPD(req: STATCODERICD9Req): Promise<any> {
        let result = [];
        try {
            const resultfindSTATCODERICD9TOPRECORDIPD = await this.PatientproceduresModel.aggregate([
                {
                    $match: {
                        orguid: 
            ...
            result = resultfindSTATCODERICD9TOPRECORDIPD
        } catch (error) {
            this.logger.error('findSTATCODERICD9TOPRECORDIPD error:', error);
        }
        return result;
    }
}
