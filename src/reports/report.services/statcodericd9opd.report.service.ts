...
@Injectable()
export class STATCODERICD9OPDReportService implements STATCODERICD9OPDReportService {
    constructor(
        private logger: ConsoleLogger,
        @InjectModel('patientprocedures') private readonly PatientproceduresModel: Model<IPatientprocedures>,        
    ) { }

    async findSTATCODERICD9OPD(req: STATCODERICD9Req): Promise<any> {
        let result = [];
        try {
            const resultfindSTATCODERICD9OPD = await this.PatientproceduresModel.aggregate([
                {
                    $match: {
                        orguid: 
            ...
            result = resultfindSTATCODERICD9OPD
        } catch (error) {
            this.logger.error('findSTATCODERICD9OPD error:', error);
        }
        return result;
    }
}
