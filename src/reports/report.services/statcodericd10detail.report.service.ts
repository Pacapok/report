...
@Injectable()
export class STATCODERICD10DETAILReportService implements STATCODERICD10DETAILReportService {
    constructor(
        private logger: ConsoleLogger,
        @InjectModel('diagnoses') private readonly DiagnosesModel: Model<IDiagnoses>,
    ) { }

    async findSTATCODERICD10DETAIL(req: STATCODERICD10Req): Promise<any> {
        let result = [];
        try {
            const resultfindSTATCODERICD10DETAIL = await this.DiagnosesModel.aggregate([
                {
                    $match: {
                        orguid: 
            ...
            result = resultfindSTATCODERICD10DETAIL
        } catch (error) {
            this.logger.error('findSTATCODERICD10DETAIL error:', error);
        }
        return result;
    }
}
