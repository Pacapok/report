...
@Injectable()
export class MCEXTENENReportService implements MCEXTENENReportService {
    constructor(
...
async findMCEXTENEN(req: MCEXTENENReq): Promise<any> {
    let result = [];
    try {
        const resultfindMCEXTENEN = await this.PatientformdetailsModel.aggregate([
            {
                $match: {
                    statusflag: "A",
    ...
    {
        $match:{
                    "formtemplates.code":"RTCOMMON130EN" //#1 Form name ???
                }
    },
            {
                $lookup: {
