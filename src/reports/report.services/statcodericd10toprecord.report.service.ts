...
@Injectable()
export class STATCODERICD10TOPRECORDReportService implements STATCODERICD10TOPRECORDReportService {
    constructor(
        private logger: ConsoleLogger,
        @InjectModel('diagnoses') private readonly DiagnosesModel: Model<IDiagnoses>,        
    ) { }

    async findSTATCODERICD10TOPRECORD(req: STATCODERICD10Req): Promise<any> {
        let result = [];
        try {
            const resultfindSTATCODERICD10TOPRECORD = await this.DiagnosesModel.aggregate([
                {
                    $match: {
                        orguid: 
            ...
            result = resultfindSTATCODERICD10TOPRECORD
        } catch (error) {
            this.logger.error('findSTATCODERICD10TOPRECORD error:', error);
        }
        return result;
    }
}
